...
public class CombinedRepository<T extends Identifiable> implements Repository<T> {

    private final Map<Class<? extends T>, Repository<? extends T>> subRepositories = new HashMap<>();
     ...
     */
    public <S extends T> void registerRepository(Class<S> clazz, Repository<S> repository) {
    ...
    private <S extends T> Repository<S> getRepository(Class<S> clazz) {
        Repository<? extends T> repository = subRepositories.get(clazz);
        if (repository == null) {
            return null;
        }
        return (Repository<S>) repository; // Safe cast because of controlled registration
    }
    ...
    public void create(T obj) {
        ...
        Repository<T> repository = getRepository(objClass);
        if (repository == null) {
    ...
    public T read(Integer id) {
        for (Repository<? extends T> repository : subRepositories.values()) {
